...
class BrandController extends Controller
{
    ...
    public function create(Request $request){
        try {
            ...
            $check=DB::select("SELECT name FROM `brand` WHERE name='$request->name'AND is_deleted=0 ");
                if ($check==true) {
                    return response()->json([
            ...
            $res=DB::insert("INSERT INTO `brand` (`name`,`slug`,`logo`,`alt_tag`) VALUES ('$request->name','$slug','$picture1','$request->alt_tag')");
    ...
    public function get(Request $request){
        ...
        $data = array();
           $totalData=\DB::table('brand')->where(['is_deleted'=>0])->count();
           $details=\DB::table('brand')->where(['is_deleted'=>0])
    ...
    public function update(Request $request){
        try {
                ...
                $check = DB::select("SELECT name FROM `brand` WHERE id<>'$request->id' AND name='$request->name' AND `is_deleted`=0");
